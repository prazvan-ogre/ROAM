...
| `trip_editorial_briefs` | One row per trip (`trip_id` primary key, `on delete cascade`), holding the creator/admin's editorial preferences for that trip's Discover/Battle content — difficulty, thematic percent split, writing style, optional narrator character name. See "Security model" point 16 below for the full contract (why this shape, who can write it, how it interacts with R7 publishing). |
...
16. **Trip editorial brief** (`20260909090000_trip_editorial_brief.sql`):
